// src/pages/DocumentView.jsx - VERSIÓN MEJORADA
...
const DocumentView = ({ documentId, onBack, notifications, tenantMetadata }) => {
    ...
    const getPrintPatientData = () => {
        ...
        return (
            <div className="grid grid-cols-2 gap-y-2 gap-x-6 text-sm text-slate-700 print:text-black mt-4 bg-blue-50 p-3 rounded-lg border border-blue-200">
                <p><User className="inline-block w-4 h-4 mr-2 align-middle text-blue-600" /><span className="font-semibold">Paciente ID:</span> {patientId}</p>
                <p className="text-right"><Stethoscope className="inline-block w-4 h-4 mr-2 align-middle text-blue-600" /><span className="font-semibold">Médico:</span> {doctorName}</p>
                <p><AlignLeft className="inline-block w-4 h-4 mr-2 align-middle text-blue-600" /><span className="font-semibold">Asunto:</span> {subject}</p>
                <p className="text-right"><Calendar className="inline-block w-4 h-4 mr-2 align-middle text-blue-600" /><span className="font-semibold">Fecha:</span> {date}</p>
            </div>
        );
    };
    ...
    const getPrintFooter = () => {
        ...
        return (
            <div className="print-footer text-center text-xs text-slate-600 mt-8 pt-4 border-t border-slate-300">
                ...
                <p className="mt-2 text-slate-500">Plataforma Inteligente de Salud potenciada por DataVoxMedical</p>
            </div>
        );
    };
    ...
    return (
        ...
        >
            ...
            <div className="flex justify-between items-center bg-white p-6 rounded-2xl shadow-lg border border-slate-200 print:hidden">
                ...
                <motion.button
                    onClick={onBack}
                    className="flex items-center space-x-2 px-4 py-2 bg-slate-100 text-slate-700 rounded-xl font-semibold hover:bg-slate-200 transition-all"
                    whileTap={{scale: 0.98}}
                >
            ...
            {/* Contenido Principal */}
            <div className="bg-white rounded-2xl p-6 shadow-xl border border-slate-200 print:shadow-none print:p-0 print:border-none">
                ...
                <div className="p-6 print:p-8 print:m-0">
                    ...
                    <div className="grid grid-cols-3 gap-4 mb-6 pb-4 border-b border-slate-200 print:hidden">
                        <div className="text-sm">
                            <span className="font-semibold text-slate-600">Paciente ID:</span>
                            <p className="font-bold text-slate-800">{documentData.clinical_meta?.patient_id || 'N/A'}</p>
                        </div>
                        <div className="text-sm">
                        ...
                        <div className="text-sm">
                            ...
                            <p className="text-slate-800">{documentData.clinical_meta?.doctor_name || 'N/A'}</p>
                        </div>
                    </div>
                    ...
                    {isEditing ? (
                        <textarea
                            ...
                            className="w-full h-96 p-4 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 resize-none text-sm font-mono bg-white"
                            ...
                            <div className="hidden print:block print:p-0 print:m-0 print:w-full print:h-full print:flex print:flex-col">
                                ...
                                <div className="text-left mb-6 mt-4">
                                    ...
                                    <div className="flex items-center space-x-4 mb-4">
                                        <div className={`w-12 h-12 bg-gradient-to-br from-blue-600 to-teal-500 flex items-center justify-center rounded-lg shadow-md`}>
                                            {React.createElement(docConfig.dual_icon, { className: `w-6 h-6 text-white` })}
                                        </div>
                                        <h1 className="text-3xl font-bold text-slate-800 tracking-tight">{docConfig.label}</h1>
                                    </div>
                                ...
                                <div className="flex-grow text-base text-slate-800 leading-relaxed mb-6 bg-white p-4 rounded border border-slate-100">
                                    {getCleanContent()}
                                </div>
                ...
                <div className="mt-6 flex justify-between space-x-3 p-6 pt-0 print:hidden">
                    ...
                    <motion.button
                        ...
                        className={`flex items-center space-x-2 px-4 py-2 rounded-xl font-semibold transition-all shadow-md ${isDocumentFinalized ? 'bg-blue-500 hover:bg-blue-600 text-white' : 'bg-slate-300 text-slate-600 cursor-not-allowed'}`}
                        ...
                        {isEditing ? (
                            <>
                                ...
                                <motion.button
                                    ...
                                    className="flex items-center space-x-2 px-4 py-2 bg-gradient-to-r from-blue-600 to-teal-500 text-white rounded-xl font-semibold shadow-lg hover:from-blue-700 transition-all"
                                    whileTap={{scale: 0.98}}
                                    disabled={isSaving}
                                >
                            ...
                            <>
                                ...
                                {!documentData.is_finalized ? (
                                    ...
                                    <motion.button
                                        ...
                                        className={`flex items-center space-x-2 px-4 py-2 rounded-xl font-semibold transition-all ${documentData.is_synced ? 'bg-slate-400 text-white cursor-default' : 'bg-gradient-to-r from-blue-500 to-teal-500 text-white hover:from-blue-600 shadow-lg'}`}
